...
coda_file_read(struct file *coda_file, char __user *buf, size_t count, loff_t *ppos)
{
	...
	ret = venus_access_intent(coda_inode->i_sb, coda_i2f(coda_inode),
				  ...
				  count, *ppos, CODA_ACCESS_TYPE_READ);
	if (ret)
               goto finish_read;

	ret = host_file->f_op->read(host_file, buf, count, ppos);

finish_read:
	venus_access_intent(coda_inode->i_sb, coda_i2f(coda_inode),
			    &cfi->cfi_access_intent,
			    count, *ppos, CODA_ACCESS_TYPE_READ_FINISH);
	return ret;
}
...
coda_file_write(struct file *coda_file, const char __user *buf, size_t count, loff_t *ppos)
{
	...
	ret = venus_access_intent(coda_inode->i_sb, coda_i2f(coda_inode),
				  &cfi->cfi_access_intent,
				  count, *ppos, CODA_ACCESS_TYPE_WRITE);
	if (ret)
		goto finish_write;
...
finish_write:
	venus_access_intent(coda_inode->i_sb, coda_i2f(coda_inode),
			    &cfi->cfi_access_intent,
			    count, *ppos, CODA_ACCESS_TYPE_WRITE_FINISH);
	return ret;
}
#else
static ssize_t
...
const struct file_operations coda_file_operations = {
	...
	.fsync		= coda_fsync,
	.splice_read	= _coda_file_splice_read,
};
