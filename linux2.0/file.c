...
coda_venus_readdir(struct inode *inode, struct file *filp, void *getdent, 
                   filldir_t filldir)
{
        int result = 0,  offset, pos, error = 0;
	int errfill;
        caddr_t buff = NULL;
        ...
        CODA_ALLOC(buff, void *, PAGE_SIZE);
        if ( ! buff ) { 
        ...
        result = read_exec(inode, filp->f_pos, buff, PAGE_SIZE, 1);
        if ( result < 0) {
        ...
        while ( pos + string_offset < result ) {
                ...
                *(debug + vdirent->d_namlen) = '\0';
                ...
                if ( vdirent->d_fileno ) {
                        ...
                        /* adjust count */
                      errfill = filldir(getdent,  name, namlen, offs, ino); 
CDEBUG(D_FILE, "ino %ld, namlen %d, reclen %d, type %d, pos %d, string_offs %d, name %s, offset %d\n", vdirent->d_fileno, vdirent->d_namlen, vdirent->d_reclen, vdirent->d_type, pos,  string_offset, debug, (u_int) offs);
...
exit:
        CDEBUG(D_FILE, "result %d, coda i->i_count is %ld for ino %ld\n", 
              error, i->i_count, i->i_ino);
        CDEBUG(D_FILE, "cache ino: %ld, count %ld\n", cnp->c_ovp->i_ino, cnp->c_ovp->i_count);
        EXIT;
        return error;
}
...
coda_release(struct inode *i, struct file *f)
{
	...
	coda_flags_to_cflags(flags, &coda_flags);
        ...
        CDEBUG(D_FILE,  
	       "RELEASE coda (ino %ld, ct %ld) cache (ino %ld, ct %ld) flags %o\n",
	...
	error = venus_release(i->i_sb, &(cnp->c_fid), coda_flags, cred);
	if ( error ) {
